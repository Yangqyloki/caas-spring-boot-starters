...
public class SafeHtmlMapPolicyValidatorTest
{
	...
	private SafeHtmlMapPolicyValidator validator;
	...
	{
		validator = new SafeHtmlMapPolicyValidator();
	...
	{
		validator.initialize(annotation);
		final boolean isvalid = validator.isValid(new HashMap<String, String>()
		{{
			put("some string", SANITIZED_HTML);
		}}, validatorContext);
		assertTrue(isvalid);
	}
	...
	public void should_be_valid_on_special_characters()
	{
		...
		final boolean isvalid = validator.isValid(new HashMap<String, String>()
		{{
			put("some string", SANITIZED_HTML_SPECIAL_VALUES);
		}}, validatorContext);
		assertTrue(isvalid);
	}
	...
	public void should_be_invalid_on_invalid_input()
	{
		...
		final boolean isvalid = validator.isValid(new HashMap<String, String>()
		{{
			put("some string", UNSANITIZED_HTML);
		}}, validatorContext);
		assertFalse(isvalid);
	}
